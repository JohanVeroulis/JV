...
amount_counts = final_df.groupBy("amount").count()
word_counts = final_df.groupBy("channel").count()

amount_counts.show()
word_counts.show()

# Filter columns where the value is greater or equal than 0.30
filtered_data = final_df.filter(final_df['amount'] >= "0.30")


#Show the filtered content of the DataFrame
#filtered_data.show()

filtered_data1 = final_df.orderBy(col("amount").desc(),col("channel"))
